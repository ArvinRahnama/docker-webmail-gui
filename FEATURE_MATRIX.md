...
**Date:** 2026-09-09 (§31b added for panel self-update) · **Status:** Authoritative. Implementation may not add a UI control that contradicts this document.
...
**Recreate does not ship for the managed container, and is not planned while the broker looks like this.** "Recreate" is not a Docker API operation — it decomposes into stop → remove → **create** → start, and `POST /containers/create` is the exact call that grants host root (arbitrary bind mounts, `Privileged`, `PidMode: host`). Withholding that call is the entire reason the broker exists, so `container.create`, `container.remove` and any recreate composite are absent from the broker's operation **vocabulary** (`packages/shared/src/broker.ts`) — not permission-gated, not admin-only, simply not expressible over the protocol. A server-side-stored container specification would not change this: the panel would still have to make the `create` call to use it. **No recreate control appears anywhere in the UI for the managed mail container** (see also §28, §31). Recreating that container stays a host-side operation done with your own deployment tooling.

_(Panel self-update, §31b, recreates the panel's own two containers — never the mail container — through internal-only `DockerApi` methods that still never become a `BrokerOperation`; the claim above is unweakened.)_
...
_(Panel self-update, §31b below, is not an exception to any claim above — it never uses `container.create`/`container.remove` as a general operation, and it never touches the mail container. See §31b for exactly what it does instead.)_

---

## 31b. Panel self-update — **Full**

Distinct from §31 in everything that matters, sharing only the page: a different pair of containers (`dwg-server`/`dwg-broker` — the panel's own two, never the mail container), a different version source (this project's own pinned GitHub Releases, not a registry-digest comparison against a floating tag), and a real, working mutation rather than a stated refusal. The docker-mailserver card above is **completely unchanged** — `/maintenance/updates` now shows two independent cards sharing nothing but the route (docs/design/self-update.md §0).

**Auto-latest, never a version picker.** The server resolves the newest published release (`SelfUpdateReleaseSourcePort` — real GitHub Releases lookup in production, a fixed fixture in development/tests, mirroring `RegistryClientPort`'s own real/fake shape) and that is the **only** version `POST /api/v1/updates/panel/apply` can ever target. The admin confirms; the admin does not choose. There is no field anywhere the web tier could use to name a different version — the broker-facing `targetVersion` is resolved entirely server-side.

**Still only the broker ever touches Docker, and `container.create`/`container.remove` are still permanently absent from the broker's operation vocabulary — §22's claim is unweakened by this feature.** The web tier holds no socket and sends nothing but a bare semver string; a job id and a status come back. What makes the recreate possible without adding a general create/remove operation: the broker composes both target image references itself, from two fixed, hardcoded repository constants, and launches a **second, detached container** — reusing the broker's own already-running image with an alternate entrypoint, publishing no new image — that performs the actual clone → stop → remove → create → start → health-poll sequence over the socket the broker already holds. `createContainer`/`removeContainer`/`pullImage` exist only as **internal-only `DockerApi` methods**, never registered as a `BrokerOperation` (`packages/shared/src/broker.ts`) — no request of any shape, however crafted, can reach them. Every recreated container's spec is **cloned from `docker inspect` of the container being replaced**, with only the image swapped — never constructed from scratch, never from anything a request carried.

**Tier 4 confirm: type-to-confirm plus the same backup-status gate Restore uses.** A real recreate can genuinely fail; the honest reason for the gate is "if this goes wrong and you need to intervene by hand, you'll want everything else in a known-good state" — not that this touches mail data (it does not). Reuses the exact same recent-verified-backup fact the docker-mailserver card already shows, not a second copy of it.

**Refuses outright while a backup or restore job is in flight**, enforced server-side before anything is ever enqueued — two operations that each expect exclusive control of the managed container or its data must never overlap with recreating the panel's own containers out from under them.

**Refuses on a source-built install, and says exactly why.** Whether an install was pulled from the registry or built locally from source (`docker compose build`, including the installer's own `DWG_IMAGE_MODE=build` path) is baked into each image at build time (`DWG_VERSION`/`DWG_IMAGE_ORIGIN` — a Dockerfile build ARG turned into image `ENV`, never a compose-level override an operator could flip). A source build can never claim to be the genuine published artifact self-update is designed to pull a newer version of, so `panel.selfUpdateCheck` reports `updatePossible: false` with the reason stated in the UI, and Apply is disabled — never hidden, never silently refused.

**Progress in two phases, reusing two existing mechanisms rather than inventing a third.** Phase A ("prepare," survivable) runs as an ordinary job — the existing job runner and SSE stream, the same machinery Backup/Restore already use — while `dwg-server` is still up. Phase B ("recreate," not survivable) reuses the panel-restart reconnect pattern: a blocking overlay, a bounded poll of `/api/v1/health`, extended to keep polling until the panel answers **at all** (any version) rather than stopping the instant it merely answers — a health-only check alone cannot tell "the new version came up" from "a rollback silently restored the old one and it's healthy too." Either way, the real verdict is never inferred from health, and never from the job's own terminal status — a self-update job left `running` by the server's own intentional replacement is expected, not a failure, and is never surfaced as one. The real verdict comes from a durable status file the updater writes to the shared data volume, read (and cleared) by a dedicated endpoint once the panel reconnects.

**Automatic rollback on a failed health check — the only rollback policy this ships.** Both containers' recreate specs are captured up front, before either is touched, specifically so a later failure always has a plan: a failed server-container health check rolls the server back alone; a failed broker-container health check rolls **both** back, since the two are never left on mismatched versions. There is no manual "roll back to a previous version" button independent of a failed apply — an explicit, recorded deferral (§31c).

**UI** `/maintenance/updates` (Panel card, separate from the docker-mailserver card) · **Broker operations** `panel.selfUpdateCheck`, `panel.selfUpdateApply` (`packages/shared/src/broker.ts`) · **API** `GET /api/v1/updates/panel`, `POST /api/v1/updates/panel/apply`, `GET /api/v1/updates/panel/last-result` · **Security** `update.self_update_started`/`_succeeded`/`_rolled_back`/`_failed` audited server-side when the outcome is discovered — never broker-side, `dwg-broker` has no database (`SECURITY.md` §4.1) · **Tests** every layer unit-tested against fakes/stubs (the broker state machine's every branch, the job's phase-A behaviour, the concurrency guard, the stale-interrupted-job-is-not-a-verdict case, the status-file read+clear endpoint, the Panel card's Tier-4 gating and reconnect poll) plus a **real-Docker-daemon CI job** (`.github/workflows/self-update.yml`, separate from lint/test/build) that builds two genuinely different versions, pushes them to a disposable local registry, and asserts a real recreate: both containers actually swap image, both come back healthy, the status file reports a real success with the correct from/to versions.

### 31c. Deferred

Recorded here rather than left silent, per working agreement 9:

- **A manual "roll back to a previous version" control**, independent of a failed apply — the owner chose to leave this for later (`docs/design/self-update.md` §9.4).
- **A real-Docker-daemon CI test of the rollback path specifically** (a forced health failure driving an automatic rollback, verified against a real daemon rather than a fake). The rollback _logic itself_ is unit-proven exhaustively — the broker's state-machine tests cover server-health-fails-rollback-server-only and broker-health-fails-rollback-both, asserting the exact Docker call sequence and that rollback recreates from the _captured_ old spec, never a re-derived one. What a real daemon adds on top of that — that a container recreated from a cloned spec genuinely boots — is already covered by the real-daemon job's success path, which recreates two containers from cloned specs and asserts they come back healthy. Forcing a genuine rollback for real needs a deliberately broken target image, a bigger, separately-reviewed follow-up.
- **Resuming or force-completing an interrupted update automatically**, if the updater process itself crashes mid-flight. The rollback plan is written to the status file before any teardown begins, so a crash from that point on leaves a documented, by-hand-recoverable record rather than silence — but `dwg-broker` does not detect or resume an incomplete update on its own next startup. An explicit, owner-accepted decision for this first version (`docs/design/self-update.md` §9.7).
- **A retry cool-down after an automatic rollback**, to protect against repeatedly retrying a target version that is itself broken — none in v1; the admin decides when to try again (`docs/design/self-update.md` §9.9).
- **`DANGEROUSLY_OVERRIDE_SELF_UPDATE_REGISTRY`** (`apps/broker/src/config.ts`) is not a product feature — it exists solely so the real-Docker-daemon CI job above can point a real broker at a disposable local registry instead of the real GHCR, since nothing is ever actually published there for that job to pull. It is never set by `docker/compose.yaml`, `installer/install.sh`, or any real deployment path; the only place it is ever set is the CI workflow's own generated, uncommitted compose override.
