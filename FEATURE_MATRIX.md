...
| `socat` presence in the DMS image                                                                   | The clamd control socket is reached through `socat`, whose availability research could not confirm. Chosen deliberately and isolated to one command builder so a replacement is a one-line change | Detect at runtime and report ClamAV as `Unknown` rather than failing obscurely                                                                                                     |
| `doveadm -f json quota get` exact key casing and whether `STORAGE` `value`/`limit` are bytes or KiB | Mailbox/Storage usage figures (§3, §7)                                                                                                                                                            | `quota-usage.ts` accepts several key spellings and assumes the long-documented Dovecot KiB convention; reports `Unknown` rather than a guessed number when the shape doesn't match |
| `postfix-{send,receive}-access.cf` exact line format `setup email restrict add` writes              | Mailbox restriction-status display (§3)                                                                                                                                                           | `postfix-access.ts` parses standard Postfix `access(5)` syntax (pattern + action); no literal captured example line was available to confirm against                               |
