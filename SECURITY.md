...
**Where each check actually lives.** These ten checks span three test runners across four workspaces (Vitest in `apps/server`/`apps/broker`, Vitest+jsdom in `apps/web`, Playwright for `e2e/`) — there is no single directory all of them could live in without relocating already-proven module coverage (M12's own working agreement: extend, don't rewrite). Two conventions make the set enumerable instead of scattered: every unit-level file written or extended specifically for this table is named `*.security.test.ts`; every browser-level one lives under `e2e/security/`. A check with no such file below was already fully proven inside its module's own tests before M12 and is cited directly.

| #   | Check                                                       | Lives in                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| --- | ----------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Injection payloads, every command builder                   | `apps/broker/src/dms/commands.test.ts` (pre-existing sweep) + `apps/broker/src/dms/command-injection.security.test.ts` (exhaustiveness manifest against `commands.ts`'s real exports, plus the gaps that manifest found: `buildAliasDeleteCommand`'s recipient, the six `doveadm sieve`/quota builders). Both moved from `apps/server` to `apps/broker` in M16 with the builders themselves — the web tier no longer constructs a command line, so the injection surface moved with it                                                                                                                   |
| 2   | Path traversal, every file-touching endpoint                | `apps/server/src/security/path-traversal.security.test.ts` (enumerates every file-touching surface in the code, closes each)                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 3   | Broker rejects out-of-enum ops / non-allowlisted containers | `apps/broker/src/app.test.ts` (`describe('container allowlist')` and the operation-enum rejection tests; pre-existing, already exhaustive)                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 4   | No Docker socket reachable from the web tier                | `eslint.config.js` (build-time `no-restricted-imports` ban on `dockerode`/`docker-modem` under `apps/server/**`) + `apps/server/src/security/docker-socket-isolation.security.test.ts` (runtime backstop: dependency manifest, source-tree scan, loaded-config shape)                                                                                                                                                                                                                                                                                                                                    |
| 5   | Cookie flags, session revocation, lockout                   | `apps/server/src/modules/auth/auth.routes.test.ts` + `auth.service.test.ts` (pre-existing, already exhaustive)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 6   | Uniform login responses                                     | `apps/server/src/modules/auth/auth.routes.test.ts` (`describe('POST /api/v1/auth/login — uniform failure')`) + `auth.service.test.ts` (timing) — pre-existing                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 7   | Security headers present; CSP not broken by the real app    | Headers: `apps/server/src/security/security-headers.security.test.ts`. CSP-against-the-real-app: `e2e/security/csp.spec.ts`, run by `playwright.config.ts`'s `chromium-security` project against a real `apps/server` with `STATIC_DIR` set — the real _built_ SPA served from the same origin as its API, in a real browser, carrying the header that server itself sends (the main E2E harness's Vite dev server cannot stand in: its HMR client is not CSP-clean). Found and fixed two real violations — see `packages/shared/src/csp.ts`.                                                            |
| 8   | Log redaction                                               | `apps/server/src/platform/logger.test.ts` (mechanism, pre-existing) + `apps/server/src/security/log-redaction.security.test.ts` (end-to-end: the real app, a real login/change-password sequence, the raw captured log text) + `apps/server/src/security/backup-remote-redaction.security.test.ts` (M13 remote-backup feature: the S3 secret access key / FTP password never appear in logs, `job_logs`, any HTTP response body, a forced-failure error, or the pre-change config snapshot, for a real config-apply + upload against a fake S3/FTP server, with a positive control per destination type) |
| 9   | Authorization enforced server-side, every mutating route    | `apps/server/src/security/route-authorization.security.test.ts` (enumerates the live app's own route table via `printRoutes`, fires every POST/PUT/PATCH/DELETE unauthenticated and CSRF-less)                                                                                                                                                                                                                                                                                                                                                                                                           |
| 10  | `npm audit` + dependency review + SBOM license gate         | `.github/workflows/security.yml` (three jobs) + `npm run check:audit` (the same production-scoped audit, locally, part of `npm run check`) + `scripts/check-licenses.mjs`                                                                                                                                                                                                                                                                                                                                                                                                                                |

Accessibility (IMPLEMENTATION_PLAN.md §2.4, run alongside this table though not one of its ten numbered items): `e2e/security/accessibility.spec.ts` — axe-core against every nav-level route plus `/login`, zero critical/serious violations, and keyboard-only completion of login and in-app navigation. Found and fixed three real violations (a contrast pairing, an invalid ARIA pattern, and a dialog with no viewport-height limit) — see `apps/web/src/styles/tokens.css`, `apps/web/src/components/metric-tile.tsx`, `apps/web/src/components/ui/dialog.tsx`.
