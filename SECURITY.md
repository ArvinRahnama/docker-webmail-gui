...
| 7   | Security headers present; CSP not broken by the real app    | Headers: `apps/server/src/security/security-headers.security.test.ts`. CSP-against-the-real-app: `e2e/security/csp.spec.ts`, run through `e2e/security/static-proxy-server.mjs` — the real _built_ SPA, real browser, real CSP header (the main E2E harness's Vite dev server cannot stand in; see that script's header). Found and fixed two real violations — see `packages/shared/src/csp.ts`. |
