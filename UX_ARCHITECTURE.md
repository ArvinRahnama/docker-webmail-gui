...
| 7   | **Three log-like destinations** — `Monitoring > Logs`, `Monitoring > Events`, `Administration > Audit Logs` — with no naming that distinguishes them.                                                                                 | Label by _whose_ record it is: **Service Logs** (Postfix/Dovecot/container output), **Docker Events** (lifecycle, moved under Docker), **Audit Log** (who did what _in this panel_). |
