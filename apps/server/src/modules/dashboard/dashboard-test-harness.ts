/**
 * Shared HTTP test harness for the M11 dashboard route tests, mirroring
 * `modules/updates/updates-test-harness.ts` exactly. Not matched by
...
export const PRIMARY_EMAIL = 'dashboard-admin@example.com';
...
export type DashboardAppOverrides = Omit<BuildAppOptions, 'config' | 'logger' | 'db'>;

/** Boots a real app (real DB, real migrations, real auth) with one enabled administrator, over caller-supplied overrides — the same overrides (`dmsDriver`, `brokerClient`, `tlsCertificateSource`, `rspamdClient`, `registryClient`) every M5-M10 driver test already uses to force a specific subsystem's behaviour. */
export async function setUpDashboardApp(
  overrides: DashboardAppOverrides = {},
