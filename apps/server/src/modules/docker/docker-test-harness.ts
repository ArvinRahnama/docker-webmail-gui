/**
 * Shared HTTP test harness for the M9 Docker module route tests, mirroring
 * `modules/security/security-test-harness.ts` exactly — same login/CSRF/
 * inject dance, extended with the `brokerClient` override `buildApp` now
...
import { FakeBrokerClient, type BrokerClient } from '../../drivers/broker/index.js';

/**
 * Builds a {@link BrokerClient} test double by overriding individual
 * methods on a real {@link FakeBrokerClient} instance. `Object.assign`
 * sets *own* properties on the instance, which JS property lookup finds
 * before the class's prototype methods — so an overridden method replaces
 * the fixture-backed default while every method not mentioned keeps
 * behaving exactly like the real fake (deterministic, in-memory, already
 * schema-valid), rather than every test having to hand-write all 18
 * `BrokerClient` methods just to stub the one or two it cares about.
 */
export function stubBrokerClient(overrides: Partial<BrokerClient> = {}): BrokerClient {
  return Object.assign(new FakeBrokerClient(), overrides);
}

export const PRIMARY_EMAIL = 'docker-admin@example.com';
...
export type DockerAppOverrides = Omit<BuildAppOptions, 'config' | 'logger' | 'db'>;

/** Boots a real app (real DB, real migrations, real auth) with one enabled administrator, over caller-supplied `brokerClient`/config overrides. */
export async function setUpDockerApp(
  overrides: DockerAppOverrides = {},
  configEnv: Record<string, string> = {},
): Promise<DockerHarness> {
