/**
 * Shared HTTP test harness for the M8 security route test files
 * (`dns.routes.test.ts`, and DKIM/TLS/Rspamd/ClamAV/Fail2ban as they
 * land), mirroring `modules/mail/mail-test-harness.ts` exactly — same
 * login/CSRF/inject dance, extended with the `DnsLookupPort` override
 * `buildApp` now accepts. Not matched by vitest's `*.test.ts` glob, so
...
export const PRIMARY_EMAIL = 'security-admin@example.com';
...
export type SecurityAppOverrides = Omit<BuildAppOptions, 'config' | 'logger' | 'db'>;

/** Boots a real app (real DB, real migrations, real auth) with one enabled administrator, over caller-supplied driver/port overrides. */
export async function setUpSecurityApp(
  overrides: SecurityAppOverrides = {},
