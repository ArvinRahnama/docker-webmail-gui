...
So the web tier holds **no socket and no Docker vocabulary**. It sends _named intents_ (`container.restart`, `logs.tail`) to the broker over an internal-only network. **There is no protocol field anywhere that can carry a bind mount, a capability, a `HostConfig`, or a container specification.** Full RCE in the web tier yields the broker's allowlist and nothing more.
...
| **Sieve**                  | Reject scripts referencing `vnd.dovecot.execute` or `sieve_pipe` — they invoke external programs.                                                                                                 |
