/**
 * The E2E harness's own in-process fake FTP server — the twin of
 * `apps/server/src/modules/backups/destinations/fake-ftp-server.ts`, duplicated
 * here for the same reason `fake-s3.ts` duplicates `fake-s3-server.ts`: the
 * Playwright typecheck project (`tsconfig.e2e.json`) references only
 * `packages/shared` and cannot import `apps/server` internals.
 *
 ...
 * 127.0.0.1 once a spec points the destination config at its port. This is
 * the sanctioned stand-in for FTP — the milestone forbids a real FTP server
 * in tests, and no spec ever talks to a real FTP server or the VPS.
 *
 * Speaks just enough of the RFC 959 control/data protocol (passive mode only)
 * for `basic-ftp` to drive the real `FtpDestination` end to end: login,
 * TYPE/PWD/CWD, PASV, LIST, STOR, APPE (upload resume), RETR (with REST for
 * download resume), SIZE, DELE, MKD.
 *
 * Plaintext only — no AUTH TLS — so specs connect with `secure: false`. FTPS
 * is a connection option the destination passes through to `basic-ftp`; the
 * transfer logic under test is identical either way, exactly as the fake S3
 * validates no signature. Auth is not validated here — any username/password
 * is accepted — the unit-level `fake-ftp-server.ts` twin is out of scope for
 * this E2E harness's own auth assertions (there are none; the security suite
 * covers redaction end to end separately).
 */
